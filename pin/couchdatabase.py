...
class CreateDB(object):
    ...
    def getAllPins(self):
        ...
        1. Get All Pins
        """
        server=server=couchdb.Server('http://192.168.0.91:5984')
        try:
            db = server.create('pins')
        except Exception:
            db = server['pins']
        pins = {}
        all_pins=[]
    ...
    def getAllBoards(self):
        ...
        2. Get All Boards
        """
        server =couchdb.Server('http://192.168.0.91:5984')
        try:
    ...
    def insertUser(self, user):
        ...
        """
        server =couchdb.Server('http://192.168.0.91:5984')  # insert hostname and port 5984 if db is not on local machine
        server_rep = couchdb.Server('http://192.168.0.93:5984')
        try:
            db = server.create('users')
        except Exception:
            db = server['users']
        try:
            db_rep = server_rep.create('users_rep')
        except Exception:
            db_rep = server_rep['users_rep']
        userid_db = 0
        for dbObj in db:
            doc = db[dbObj]
            userid_db = doc['id']
        ...
        server.replicate('http://192.168.0.91:5984/users','http://192.168.0.93:5984/users_rep',continuous=True)
        doc = db[doc_id]
        userData = {}
    ...
    def insertPin(self, pin):
        ...
        try:
            ...
            db = server['pins']
        try:
            db_rep = server_rep.create('pins_rep')
        except Exception:
            db_rep = server_rep['pins_rep']
        pin_id = db.save({'pinid': pin._pinid,'pinname': pin.pinname, 'pinurl': pin.pinurl, 'boardid':[]})
        server.replicate('http://192.168.0.91:5984/pins','http://192.168.0.93:5984/pins_rep',continuous=True)
        pin_dict = {}
    ...
    def insertBoard(self, board):
        ...
        try:
            ...
            db = server['boards']
        try:
            db_rep = server_rep.create('boards_rep')
        except Exception:
            db_rep = server_rep['boards_rep']
        boardid_db = 0
        for dbObj in db:
            doc = db[dbObj]
        ...
        server.replicate('http://192.168.0.91:5984/boards','http://192.168.0.93:5984/boards_rep',continuous=True)
        board_dict = {}
    ...
    def insertComments(self,comments):
        ...
        try:
            ...
            db = server['comments']
        try:
            db_rep = server_rep.create('comments_rep')
        except Exception:
            db_rep = server_rep['comments_rep']
        comment_id = db.save({'comment': comments.comment, 'usercomid':comments.usercomid, 'pinid':comments.pincommentid})
        server.replicate('http://192.168.0.91:5984/comments','http://192.168.0.93:5984/comments_rep',continuous=True)
        return "Done"
