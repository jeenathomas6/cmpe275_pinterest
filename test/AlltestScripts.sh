...
'{"username":"testu","password":"'$password'"}'  http://localhost:8080/v1/login)
userJson=$(echo $arr | awk -F'STATUS' '{print $1}')
...
arr=$(curl -s -w 'STATUS%{http_code}' -H "Content-Type: multipart/related" \
 --form "content=@velocity.jpg;type=image/jpeg" http://localhost:8080/v1/user/$user_id/pin/upload)
